...
#install.packages("DescTools", lib=local_lib, repo='http://cran.us.r-project.org')
...
combined1$UTR_clip_mapping_POS <- paste0(combined1$ref_id.x,":", combined1$ref_start, "-", combined1$ref_end)
combined1$Mate_mapping_POS <- paste0(combined1$ref_id.y,":", combined1$MATE_POS_start, "-", combined1$MATE_POS_end)
combined1 <- combined1[order(read_id, ref_id.x)]
combined1 <- unique(combined1)

combined1[,ref_id.y:=NULL]
setnames(combined1, "ref_id.x", "ref_id")
